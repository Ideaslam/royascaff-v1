...
**RoyaScaff = `.cursor/royascaff/engine/` only.** The blueprint folder `project/` is **not** shipped empty. The engine creates it when a build or reverse-engineer starts. Layout contract: `.cursor/royascaff/engine/project-layout.md`.
...
**Resuming blueprint overview?** If `project/status.md` exists, read it for `done` / `partial` / `planned` / `deferred` on main. See `.cursor/royascaff/engine/conventions.md` → **Build Status** and **Main vs pack vs index**.
...
|-------|---------|-----------|
| 0–4 | Initial Build (design + REQ-INIT packs) | `.cursor/royascaff/engine/flows/initial-build.md` |
| 5 | Change Mode (features / plan-impacting work) | `.cursor/royascaff/engine/flows/change-mode.md` |
| P | Polish (visual/style/copy only) | `.cursor/royascaff/engine/flows/polish.md` |
| 6 | Bug Fix | `.cursor/royascaff/engine/flows/bug-fix.md` |
| R | Reverse-Engineer existing codebase | `.cursor/royascaff/engine/flows/reverse-engineer.md` |
...
See `.cursor/royascaff/engine/conventions.md` for defaults that specs inherit (route prefix, auth model, envelope, pagination, UI states, build status, pack-status).

## Two Zones

- **`.cursor/royascaff/engine/`** — the product: this router, flows, templates, rules, and `project-layout.md`.
...
- **Layout contract**: `.cursor/royascaff/engine/project-layout.md`
- **Templates**: `.cursor/royascaff/engine/templates/`
