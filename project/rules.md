...
Adapter pattern via `PaymentProvider` in `src/integrations/payment/`; PayUp remains the default provider. A server-priced immutable `BillingInvoice` is created before checkout and may have multiple append-only `Payment` attempts. Public return/webhook handlers correlate by opaque attempt reference, verify the provider session server-side, atomically settle the attempt and invoice, then enqueue a durable lifecycle event in the same transactional/outbox boundary. The lifecycle processor, not the browser callback, applies the invoice exactly once. Provider secrets/tokens and hosted redirect URLs are never exposed by general invoice/payment DTOs or logs; card/CVV data is never stored. Cancelled checkout only fails that attempt and never suspends a user or changes entitlement. Amount, currency, action, plan snapshots, period boundaries, proration inputs, lifecycle version, expiry, and idempotency key come from the server. Expired, void, refunded, chargeback, or superseded invoices cannot activate a plan. Duplicate or delayed callbacks return the already-derived result and safely re-enqueue an unapplied paid invoice when necessary.

### RULE-SUB-002: Entitlement Period Continuity and Usage
Module: Subscriptions · Feature: Periods and Limits
There is exactly one `UserSubscription` per workspace and exactly one current immutable `SubscriptionPeriod`. Free periods are consecutive exact 30-day windows; paid period length comes from the plan's billing interval. Usage belongs to the period and resets only by creating the next period. Reissuing a request, retrying a callback, changing auto-renew, cancelling a downgrade, or returning to free never recreates the subscription or resets the current period. Quota checks use one conditional atomic reservation/increment against the expected current period and fail closed when entitlement cannot be resolved. Downgrades never delete data; existing resources remain readable while new limited actions are blocked above the new limit.

### RULE-SUB-003: Plan Change, Renewal, and Grace Lifecycle
Module: Subscriptions · Feature: Upgrade, Downgrade, Renewal
Only workspace owners may operate self-service billing. Free-to-paid creates a full paid invoice and starts one new paid period after verified settlement. Paid-to-paid upgrade creates a normalized prorated positive-difference invoice for the remaining current period; settlement applies the higher plan immediately without changing period end or usage. Downgrades, including paid-to-free, are scheduled for period end, displayed with target/effective time, and cancellable. A paid downgrade target receives a next-cycle invoice before the boundary and applies after settlement; if unpaid, current paid access follows the same grace-to-free path. Paid cancellation means `autoRenew=false`; paid access continues to period end. If a due renewal/downgrade invoice remains unpaid, keep paid access through `SUBSCRIPTION_PAYMENT_GRACE_DAYS` (integer, default 7, startup-validated), then transition to a new free period. Admin overrides require admin role and reason. Account suspension remains separate.

### RULE-SUB-004: Lifecycle Idempotency and Financial Immutability
Module: Subscriptions · Feature: Reliability and Audit
Invoice rows, payment attempts, subscription periods, and financial audit events are append-only. Every command has a unique workspace-scoped idempotency key and expected subscription lifecycle version. Only one actionable upgrade, renewal, or scheduled downgrade may target a lifecycle version; newer commands explicitly supersede compatible older invoices. State transitions use transactions or compare-and-set plus durable outbox. Workers and reconciliation jobs are retry-safe, and every transition records actor/source, correlation ID, before/after state, invoice ID, and period ID. No admin endpoint may arbitrarily edit or delete settled financial history.
...
Dashboard creation, CSV upload, data refresh, and sync must reserve capacity via `SubscriptionLimitService` after auth and before the limited write. The reservation is a single conditional atomic update on the current `SubscriptionPeriod`; rollback/compensation occurs when the protected operation fails. Return 403 with a clear current/limit/period response. New limits use `SubscriptionLimitRegistry` handlers—no duplicated logic. Entitlement/resource lock is resolved before the limit check and resolution failures deny the write.
