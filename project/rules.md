...
All emails use `src/integrations/mail/`—never the MailJet SDK directly. HTML templates use Roya brand colors and never include secrets/internal IDs. Ordinary product email failure may remain best-effort, but Plan-retirement email is durable: one deduplicated owner delivery per retirement schedule, persisted status/attempt count, exponential retry, allowlisted failure code, and sent/failed audit event. It never blocks retirement, and the subscription-page warning derives from Plan state rather than email success. Email is never sent synchronously on the retirement request path.
...
Adapter pattern via `PaymentProvider`; PayUp remains default. A server-priced immutable `BillingInvoice` snapshots exact Plan and Package versions, commercial/entitlement inputs, target access period, proration, lifecycle version, and idempotency before checkout. Multiple append-only attempts may retry it. Server verification, settlement, lifecycle delivery, secret exclusion, cancelled-attempt behavior, and late/superseded callback rules remain unchanged.

### RULE-SUB-002: Entitlement Period Continuity and Usage
Module: Subscriptions · Feature: Periods and Limits
There is one `UserSubscription` per workspace, one current immutable billing/access `SubscriptionPeriod`, and one independent current immutable `SubscriptionUsagePeriod`. Plan billing interval advances access/invoices; Package quota interval advances counters. A Package interval supports positive `day|month|year` count; default Free is exact rolling `30 day`. Billing renewal alone never resets quota. One compare-and-set next usage period resets counters exactly once. Quota reads the usage period's immutable Package snapshot, never a live catalog document, and fails closed. Existing over-limit resources remain readable.

### RULE-SUB-003: Plan Change, Renewal, and Grace Lifecycle
Module: Subscriptions · Feature: Upgrade, Downgrade, Renewal
Only workspace owners operate self-service billing. Direction is Package tier: higher is upgrade, lower is downgrade, and a different Plan on the same Package/tier is replacement. Free-to-paid starts one access and one usage period. Paid upgrade applies immediately after verified proration while preserving access end, usage end, and used quota; the target Package cadence begins next usage window. Downgrade/replacement schedules for access-period end and does not reset quota unless its boundary is due. Renewal/grace rules remain for renewable Plans. A Plan cannot start a period on/after its `retireAt`; retired fallback gets no extra retired-Plan grace. Account suspension remains separate.

### RULE-SUB-004: Lifecycle Idempotency and Financial Immutability
Module: Subscriptions · Feature: Reliability and Audit
Packages/Plans become immutable after publish/reference. Invoice rows, attempts, access periods, usage periods, and financial/catalog audit history are append-only. Clone version allocation, publish/unpublish, retirement, notifications, access transitions, and quota rollover use idempotency plus compare-and-set/unique indexes. Every admin catalog command records actor, reason, correlation, and before/after identity. No admin endpoint may arbitrarily edit referenced catalog or financial history.

### RULE-SUB-005: Billing API Security
Module: Subscriptions · Feature: Billing Authorization
Customer billing endpoints require JWT, active workspace membership, `workspace-owner` role, per-workspace rate limiting, and server-derived workspace identity. Public PayUp returns accept only opaque correlation input and never trust browser outcome fields. Customer responses use allowlisted DTOs and never include provider session tokens, internal redirect URLs, secrets, raw gateway payloads, or other workspaces' records. Admin views require platform-admin authorization; manual settlement/refund/chargeback/reconciliation requires a reason and immutable audit event.

### RULE-SUB-006: Package/Plan Publication and Retirement
Module: Subscriptions · Feature: Catalog Versioning
Package owns entitlement identity; Plan owns commercial identity. Draft/unreferenced records may be edited/deleted. Published/referenced identity changes require cloning the next family version. `isPublished=false,isActive=true` is hidden but grandfather-renewable. Retirement requires at least 30 days, immediately unpublishes, may be cancelled/rescheduled before effect, and never revokes a started period. Renewals may start before but not on/after `retireAt`. Exactly one active published explicit default Free Plan must exist. At expiry, retired paid without a paid replacement moves to default Free; retired Free switches at the next usage boundary. Only platform admins manage catalog state, with reason/idempotency/audit.

### RULE-SUB-007: Catalog Naming and Uniqueness *(change-077)*
Module: Subscriptions · Feature: Catalog Versioning
A Plan's client-facing label is admin-owned in `displayName`/`displayNameAr`/`displayDescription`/`displayDescriptionAr` and is **never** overwritten by the Package→Plan compatibility mirror; the mirror may only maintain `name*`/`description*`/limit fields. Every surface — customer portal, admin panel, retirement notices, gateway product names, and period/invoice snapshots — resolves labels through the single shared resolver `displayName` → `plan.name` → `package.name` (Arabic chain under an Arabic locale, snapshot fallback for pre-change rows), so an empty display field is valid and reproduces existing behaviour. Uniqueness is scoped to what is currently on offer: Package `name` (case-insensitive, trimmed) and `tierRank` are unique among non-archived Packages, and a resolved Plan label is unique among active+published Plans — archived, retired, and superseded versions keep their historical labels and those labels may be reused. The Plan-label check runs at **publish**, not on draft writes, and exempts other versions of the same `familyKey`: a v2 draft necessarily inherits v1's label, so enforcing it earlier or across the family would make versioning impossible. Conflicts return stable codes (`SUBSCRIPTION_PACKAGE_NAME_TAKEN`, `SUBSCRIPTION_PACKAGE_TIER_TAKEN`, `SUBSCRIPTION_PLAN_DISPLAY_NAME_TAKEN`) that both frontends localize as field-level errors.

### RULE-SUB-008: The Default-Free Role Is Transferable *(change-077)*
Module: Subscriptions · Feature: Catalog Versioning
Exactly one active published Plan carries `isDefaultFree`, and publication only ever **grants** that role — `publishPlan` no-ops on an already published Plan, so the flag can never be moved by re-publishing. Moving it is therefore its own admin command (`setDefaultFreePlan`, `EP-SUB-34`): the target must be a free, active, published, non-retiring Plan on an exact 30-day Package, and the outgoing holder releases the role in the same operation. Without this command a newly published Free version can never take over, which in turn makes the outgoing Free Plan permanently impossible to hide or retire — both of those actions are blocked while a Plan holds the role. The swap releases the old holder before claiming the role, so the partial unique index on `{isDefaultFree, isActive, isPublished}` is never violated; it runs in a transaction where the deployment supports one and falls back to the same ordered writes on standalone MongoDB.
...
Dashboard creation, CSV upload, data refresh, and sync reserve capacity via `SubscriptionLimitService` after auth and before the limited write. Reservation is one conditional atomic update on the current `SubscriptionUsagePeriod` using its Package snapshot; rollback/compensation occurs when the protected operation fails. Return 403 with clear current/limit/usage-period data. New limits use registry handlers; entitlement or usage-window resolution failure denies the write.
...
All create/update/delete/share/export/login events → immutable `auditlogs` via `AuditService`. Controllers never write directly. Audit logs never deletable via API. Subscription and financial transitions must persist their audit/outbox event in the same atomic boundary as the state transition; fire-and-forget logging is allowed only for non-financial telemetry.
...
### RULE-GLOBAL-013: Migrations vs Seeds *(change-077)*
Production has no direct database access — only the backend does — so every data change ships through the backend image, and the two mechanisms are never mixed. **Migrations** are one-time transformations of existing data: each is an ordered module under `src/database/migrations/`, is applied only if absent from the `schema_migrations` ledger, is serialized across replicas by an atomic advisory lock on the unique `name` index, runs in a `k8s.deploy` `initContainer` so a non-zero exit prevents the app from booting against an unmigrated database, and is gated by `MIGRATIONS_ENABLED` (default `true`; `false` logs and exits `0` without connecting). A failed migration is not recorded and releases its lock so the next deploy retries. Applied migration modules are append-only — never edited or re-purposed. **Seeds** are idempotent, insert-if-missing baseline rows keyed on a natural identity, run in-process via `OnModuleInit`, must never update, overwrite, archive, or delete existing rows, must never block boot on failure, and are not recorded in the ledger. A seeder must never be used to fix existing data, and a migration must never be used to create baseline rows.
