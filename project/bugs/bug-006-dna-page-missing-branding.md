# Bug #006 — DNA page missing branding section
...
## Root Cause
`project-dna.component.ts` `dnaSections` hardcodes display keys and omits `branding`. i18n `projects.dnaKeys` also lacks a `branding` label. Backend inject/reconcile (`dna-passthrough` / `resolveBrandingColors`) is fine — this is a FE render gap after change-022.
