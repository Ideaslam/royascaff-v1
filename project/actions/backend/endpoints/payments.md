...
| EP-PAY-04 | GET | /api/v1/payments/invoices | JWT+admin | filters + pagination | 200 paginated `AdminInvoiceDto` with immutable Plan + Package versions/snapshots | SVC-PAY-INV.list() | Immutable ledger |
| EP-PAY-05 | GET | /api/v1/payments/invoices/:id | JWT+admin | `:id` | 200 `AdminInvoiceDetailDto` with immutable Plan + Package versions/snapshots | SVC-PAY-INV.get() | Safe attempt history |
...
**Contract rules:** no general PATCH/DELETE. Corrections use explicit append-only transitions with admin reason. Every invoice preserves the exact commercial Plan and entitlement Package identities/snapshots used for calculation; later catalog versions never rewrite ledger history. Public callbacks ignore browser outcome/amount/plan/workspace fields. Repeated verified callbacks safely repair missing lifecycle delivery.
