...
| EP-DATA-49 | POST | /api/v1/data/datasets/:id/refresh-available-columns | JWT | `:id` | 200 `DatasetDto` | SVC-DATA-DS.refreshAvailableColumns() | Add-column: re-discover from source into `availableColumns`; AI identify for **new columns only**; does not auto-prune or sync *(change-055)* |
| EP-DATA-22 | POST | /api/v1/data/datasets/:id/discover-schema | JWT | `:id` | 200 `DatasetDto` | SVC-DATA-DS.discoverSchemaWithAiProposal() | Re-discovers into `availableColumns` + column-identify selection AI + mapping proposal *(change-022, change-055)* |
...
- [EP-DATA-49] Used by dataset-detail **Add column**: live source rediscovery + AI for new cols only; user must confirm selection (EP-DATA-48) then manual sync for OLAP data.
- [EP-DATA-22] Can be called again at any time to refresh discovery (e.g. after re-uploading a CSV). Writes `availableColumns`, runs column-identify + mapping proposal. Does not silently replace a confirmed pruned `schema` without merge rules.
