...
- `refreshAvailableColumns(id, workspaceSlug)` — *(change-055)* Add-column path: re-runs `connector.discoverSchema`, merges into `availableColumns`, runs column-identify AI **for new columns only**; does not auto-select new cols into live `schema` until user confirm + manual sync. Backs EP-DATA-49
