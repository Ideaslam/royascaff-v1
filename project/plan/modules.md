...
2A. **Plan Client-Facing Labels** [both] *(change-077)* — a Plan owns four admin-editable display fields (`displayName`/`displayNameAr`/`displayDescription`/`displayDescriptionAr`) that the Package compatibility mirror never overwrites. One shared resolver — `displayName` → `plan.name` → `package.name`, with the Arabic chain under an Arabic locale — is used by every surface, so two Plans sharing a Package can present distinct customer-visible names while empty fields reproduce today's Package-derived labels. Uniqueness is enforced on the resolved label within active+published scope only; Package `name` and `tierRank` are unique among non-archived Packages.
2B. **Guided Versioning of a Live Package** [admin-panel + backend] *(change-077)* — because published/referenced catalog rows are immutable, `POST /packages/:id/new-version` clones a Package together with its linked Plans into one editable draft set. Existing subscribers stay on their current version and nothing reaches customers until the new Plan is published, which turns the previous dead end ("can't create/edit because users exist") into an explicit, discoverable action.
2C. **Default-Free Catalog Seed** [backend] *(change-077)* — `SubscriptionCatalogSeeder` runs `OnModuleInit` and inserts the mandatory default Free Package + Plan only when no active published default-Free Plan exists, insert-if-missing by `familyKey`. It never updates, overwrites, archives, or deletes existing catalog rows, so a fresh database boots usable and an existing one is untouched.
...
### Features
1. **Manage Entitlement Packages** [both] — `/app/packages` manages bilingual Package drafts, limits/features, included users, quota cadence, lineage/version, cloning, archive, and linked Plans. Published/referenced identity is read-only; changing a live Package uses **Create new version**.
2. **Manage Commercial Plans** [both] — `/app/plans` manages Package-linked pricing/billing offers, client-facing display labels, commercial lineage/version, publish/unpublish, clone, explicit default-Free identity, and cancellable/reschedulable future retirement. It never edits Package limits.
2A. **Task-Oriented Catalog UI** [admin-panel] *(change-077)* — both catalog pages use text-labelled actions (one primary per row plus an overflow menu) instead of icon-only controls, show a lifecycle stage column with a next-step hint, disable blocked actions with the reason inline, group dialog fields into labelled sections with plain-language help, and cross-validate `planType` against `priceAmount`/currency client-side so the API's pricing rules can't be violated from the form. All stable catalog conflict codes render as localized field-level errors.
...
### Features
1. **Payment Provider Interface** [backend] — provider-agnostic interface (`PAYMENT_PROVIDER` env var); PayUp provider creates hosted sessions and performs authoritative server-side status lookup for invoice-bound attempts. Provider calls remain isolated; keys are env-only; lifecycle changes occur only after verified invoice settlement and durable event persistence.
...
## S14. Database Migrations *(change-077)*
- Scope: BE only — `src/database/migrations/` + `k8s.deploy` `initContainer`
- Audience: system (deploy-time); no HTTP surface
- Entities: `schema_migrations`

### Features
1. **Ledger-Backed Migration Runner** [backend-only] — a standalone runner on the API image discovers ordered migration modules and applies only those absent from the `schema_migrations` ledger, so each migration runs exactly once ever and repeat deploys are a no-op. It exists because production database access is available only through the backend.
2. **Concurrency-Safe Advisory Lock** [backend-only] — acquisition is an atomic upsert against the unique `name` index with stale-lock reclaim, so under `replicas: 2` one runner applies while the other observes and exits `0`.
3. **Fail-Closed Deploy Gate** [backend-only] — the runner is an `initContainer`, so a failing migration is not marked applied, releases its lock so the next deploy retries, and exits non-zero — the app container never starts against an unmigrated database.
4. **`MIGRATIONS_ENABLED` Off-Switch** [backend-only] — default `true`; setting it `false` makes the `initContainer` log and exit `0` without connecting, which is how migrations are turned off after a rollout without editing manifests or deleting code.
5. **Migrations vs Seeds Separation** [backend-only] — migrations are one-time, ledgered, and delivered pre-boot; seeds stay idempotent, insert-if-missing, and in-process via `OnModuleInit`. Applied migration modules are append-only and are never edited afterwards.
