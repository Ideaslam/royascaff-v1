...
Desired behavior:
- Register pipeline types on the engine (change-019):
  - `dashboard-generate`: datasets → widget definitions (today's main flow, now a sequence of steps: gather dataset schemas/mappings → select widgets → build query definitions → build filters (hook to change-021)).
...
- **Multi-datasource**: generation and query building read **all datasets linked to the dashboard** (via the M:N from change-015), across sources, using canonical views where semantic flags match.
- Widget `queryDefinition` is stored as the **dialect-neutral query spec** and executed via the analytics-store (change-014), which compiles it to the workspace's active engine (**ClickHouse or BigQuery**); filters inject into the spec, not raw SQL.
- Redis result-cache (change-014) is used for widget data; invalidation hooks to sync completion.

Out of scope: filter-value computation internals (change-021 — this consumes its `build-filters` step/API), connectors (Phase C).
...
## Notes (optional)
- Depends on: 014, 015, 016, 017, 019, and 021 (filters) for the build-filters step.
