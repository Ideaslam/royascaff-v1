...
Out of scope: prompt storage (change-004), pipeline steps that consume the provider (change-006). Adding a second concrete provider is optional/future — only the abstraction + Anthropic refactor are required here.
...
## Notes (optional)
- Depends on: none (independent). Consumed by: 006 (pipeline steps), 007 (dashboard pipelines).
