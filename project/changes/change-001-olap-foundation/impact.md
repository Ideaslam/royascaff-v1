...
| `workspace.schema.ts` | `olapEngine` field added in change-002 (not this CR); `AnalyticsStoreService` will read it from the workspace object passed by callers. |
...
roya-ai-dynamo-api/src/integrations/olap/
  ...
  dialect-partials/
    dialect-clickhouse.ts           # SQL capabilities context (for prompts, change-004)
    dialect-bigquery.ts
...
- **Do not touch** `dashboards/services/dashboards.service.ts` `executeAggregation()` in this CR — that migration is change-007.
