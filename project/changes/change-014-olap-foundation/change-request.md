...
- **Per-account/workspace engine selection** — the active engine is resolved from the workspace's `olapEngine` field (set at account creation — see change-015). All analytics-store operations resolve the engine via the registry for the current workspace. `datasetId`/`workspaceSlug` are opaque inputs here.
...
## Notes (optional)
- Depends on: none (first). Foundation pair with 015 (which adds the `olapEngine` field on the workspace/account). Enables all subsequent Phase B changes.
...
- Switching a workspace's engine after data exists requires a full re-sync into the new engine (note for change-029).
