...
## Description
Build the **generic pipeline engine** that runs ordered, editable **steps** — each step is **AI-based or programmatic** (A6). This is the reusable execution core for ingestion cleaning and (via change-007) dashboard operations.

Desired behavior:
- **Step contract**: `id`, `type`, `config`, `position` (pre-load | post-load | standalone), `enabled`; `run(context) -> context`. A step may be **programmatic** (e.g. `type-cast`, `dedupe`, `trim`, `apply-mapping`, `build-aggregates`) or **AI-based** (e.g. `profile-data`, `clean-data` where AI generates a cleaning transform). AI steps resolve an **AI provider (change-003)** and a **prompt template key (change-004)**.
- **OLAP-engine awareness (change-001):** steps never emit raw dialect SQL directly. They operate through the **`OlapEngine` strategy** (staging/query/aggregate ops) using the **neutral query spec**; where a step must generate free-form SQL (advanced `clean-data`), it passes the workspace's `dialect` context to the prompt (change-004) so the output matches the active engine (ClickHouse or BigQuery).
...
- Steps operate against an OLAP **staging table** (change-001, ClickHouse or BigQuery) and can act **before or after** load.
- **PipelineRun** records execution: per-step status, timings, inputs/outputs summary, errors — linked from `SyncRun`.
- The `DataSyncProcessor` (change-005) invokes `PipelineEngine.run('ingest', dataset, context)` instead of a hardcoded extract→load.

Out of scope: dashboard-operation pipeline types (change-007), concrete connectors (Phase C), filter store internals (change-008 — provides the `build-filters` step it registers).
...
6. `DataSyncProcessor` runs syncs through the engine (replacing any pass-through from change-005) with no regression.

## Notes (optional)
- Depends on: 001 (staging/load), 002 (dataset), 003 (AI provider), 004 (prompts), 005 (processor).
