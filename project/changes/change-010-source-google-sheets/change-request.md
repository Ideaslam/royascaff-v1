...
- `syncPolicy` supports **scheduled** refresh (hourly/daily) + manual refresh; incremental not required (sheet re-read), but guard against schema/column moves (drift → change-016).
...
## Notes (optional)
- Depends on: 001–008. Reuses Google OAuth config already present in backend config.
